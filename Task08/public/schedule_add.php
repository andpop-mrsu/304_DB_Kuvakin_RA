<?php
/**
 * Добавление записи в график работы мастера
 */
    ...
    if (empty($day_of_week) || empty($start_time) || empty($end_time)) {
        ...
        try {
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmt->execute([$mechanic_id, $day_of_week, $start_time, $end_time, $is_active]);
            ...
            <div class="form-group">
                ...
                <select id="day_of_week" name="day_of_week" required>
                    ...
                    <?php foreach ($days as $num => $name): ?>
                        <option value="<?= $num ?>" 
                                <?= (isset($_POST['day_of_week']) && $_POST['day_of_week'] == $num) ? 'selected' : '' ?>>
