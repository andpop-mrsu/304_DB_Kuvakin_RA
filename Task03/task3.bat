...
echo "--------------------------------------------------"
sqlite3 movies_rating.db -box -echo "SELECT u.name AS expert_name, m.title, m.year, r.rating, DATE(r.timestamp, 'unixepoch') AS rating_date FROM ratings r INNER JOIN users u ON r.user_id = u.id INNER JOIN movies m ON r.movie_id = m.id ORDER BY u.name, m.title, r.rating LIMIT 50;"
echo " "
...
echo "--------------------------------------------------"
sqlite3 movies_rating.db -box -echo "SELECT m.title, m.year, COUNT(*) AS high_ratings_count FROM movies m INNER JOIN ratings r ON m.id = r.movie_id INNER JOIN users u ON r.user_id = u.id WHERE m.genres LIKE '%Comedy%' AND m.year > 2000 AND u.gender = 'M' AND r.rating >= 4.5 GROUP BY m.id, m.title, m.year ORDER BY m.year, m.title;"
echo " "
